...
int main()
{
	...
	printf("------Testing Outpost for 1000 hands------\n");
	for (n=0; n<1000;n++)
	{	int successFlag=1; 
		...
		printf("Player %d's turn to play Outpost\n", p);
		...
		int checkFunction = cardEffect(outpost, choice1, choice2, choice3, &G, handpos, &bonus);
		
		//assert that it returned 0 so there was no internal problem 
		assert(checkFunction == 0); 

		//check that the number of Actions was not affected 
		...
		if (G.numBuys != testG.numBuys)
			{printf("Test failed, incorrect # of buys! Expected: %d Actual: %d\n", testG.numBuys, G.numBuys); successFlag=0;}
		else {printf("Test passed!\n");}
		//check that the Outpost Played status was not changed 
		printf("-----Checking Outpost Played Status------\n");
		if (G.outpostPlayed != testG.outpostPlayed+1)
			{printf("Test failed, incorrect Outpost status!\n"); successFlag=0;}
		else {printf("Test passed!\n");}

		//check that number of played cards was increased by 1 as outpost calls discardCard 
		...
		printf("------Checking Player Hand Count------\n");
		
			if(G.handCount[p] != testG.handCount[p]-1)
			{
				printf("Test failed, player %d's hand count changed! Expected: %d Actual: %d \n",p, testG.handCount[p],G.handCount[p]);
				successFlag=0;
			}
			else {printf("Test passed for player %d\n", p);}
