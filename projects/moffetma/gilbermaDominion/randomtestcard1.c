/*
Max Moffett 
randomtestcard1- Village - updated for Assignment 5 
*/
...
int main()
{
	...
	printf("------Testing Village for 1000 hands------\n");
	for (n=0; n<1000;n++)
	{	
		int successFlag=1; 
		...
		printf("Player %d's turn to play Village\n", p);
		...
		int checkFunction = cardEffect(village, choice1, choice2, choice3, &G, handpos, &bonus);
		
		//assert that it returned 0 so there was no internal problem 
		assert(checkFunction == 0); 

		//check that the number of Actions was increased by 2 when compared to testG which did not play village 
		...
		//check that number of played cards was increased by 1 as village calls discardCard 
